...
SERVER_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' pqc_server 2>/dev/null || true)
SERVER_PORT="4433"
if [ -z "${SERVER_IP}" ]; then
  echo "[!] No se pudo resolver IP de pqc_server (¿contenedor levantado?)" >&2
  exit 1
fi
...
percentile_field_value() {
  local filter="$1"
  local field="$2"
  local percentile="$3"
  { tshark -r "${PCAP}" -Y "${BASE_FILTER} && (${filter})" -T fields -e "${field}" 2>/dev/null || true; }     | awk -v p="${percentile}" '
      NF {
        for (i=1; i<=NF; i++) {
          if ($i != "") {
            values[++n] = $i + 0
...
TCP_ACK_RTT_P50_S=$(percentile_field_value 'tcp.analysis.ack_rtt' 'tcp.analysis.ack_rtt' 50)
TCP_ACK_RTT_P95_S=$(percentile_field_value 'tcp.analysis.ack_rtt' 'tcp.analysis.ack_rtt' 95)
TCP_ACK_RTT_P99_S=$(percentile_field_value 'tcp.analysis.ack_rtt' 'tcp.analysis.ack_rtt' 99)
if [ -n "${TCP_ACK_RTT_P50_S}" ]; then TCP_ACK_RTT_P50_MS=$(awk -v v="${TCP_ACK_RTT_P50_S}" 'BEGIN {printf "%.3f", v*1000}'); else TCP_ACK_RTT_P50_MS=""; fi
if [ -n "${TCP_ACK_RTT_P95_S}" ]; then TCP_ACK_RTT_P95_MS=$(awk -v v="${TCP_ACK_RTT_P95_S}" 'BEGIN {printf "%.3f", v*1000}'); else TCP_ACK_RTT_P95_MS=""; fi
if [ -n "${TCP_ACK_RTT_P99_S}" ]; then TCP_ACK_RTT_P99_MS=$(awk -v v="${TCP_ACK_RTT_P99_S}" 'BEGIN {printf "%.3f", v*1000}'); else TCP_ACK_RTT_P99_MS=""; fi
...
# Percentiles por stream para latencia CH->SH y duración de handshake
HS_APPDATA_FALLBACK_MAX_MS="${HS_APPDATA_FALLBACK_MAX_MS:-6000}"
THROUGHPUT_WINDOW_S="${THROUGHPUT_WINDOW_S:-1}"
PERCENTILES_OUTPUT=$(awk -v max_fallback_ms="${HS_APPDATA_FALLBACK_MAX_MS}" '
  function compute(vals, n, p,   rank, lo, hi, frac) {
    if (n==0) return ""
    if (n==1) return vals[1]
    rank = (p/100)*(n-1)+1
    lo = int(rank); hi = lo+1; if (hi>n) hi=n
    frac = rank-lo
    return vals[lo] + frac*(vals[hi]-vals[lo])
  }
  function push_lat(v) {lat[++nlat]=v; lat_sum+=v}
  function push_hs(v) {hs[++nhs]=v; hs_sum+=v}
  function push_tput(v) {tput[++ntput]=v; tput_sum+=v; if (v>tput_max) tput_max=v}
  BEGIN{}
  FNR==NR {
    # file1: stream,first_CH_epoch
    if (NF>=2 && !($1 in ch)) ch[$1]=$2+0
    next
  }
  ARGIND==2 {
    # file2: stream,first_SH_epoch
    if (NF>=2 && ($1 in ch) && !($1 in sh) && ($2+0)>=ch[$1]) sh[$1]=$2+0
    next
  }
  ARGIND==3 {
    # file3: stream,first_FIN_epoch
    if (NF>=2 && ($1 in ch) && !($1 in fin) && ($2+0)>=ch[$1]) fin[$1]=$2+0
    next
  }
  ARGIND==4 {
    # file4: stream,first_client_appdata_epoch
    if (NF>=2 && ($1 in ch) && !($1 in appc) && ($2+0)>=ch[$1]) appc[$1]=$2+0
    next
  }
  END {
    ch_total=0
    ...
    for (s in ch) {
      ch_total++
      if (s in sh) push_lat((sh[s]-ch[s])*1000)
      endt=""
      if (s in sh) {
        if (s in fin) {
          endt=fin[s]
        } else if (s in appc) {
          gap_ms=(appc[s]-ch[s])*1000
          if (gap_ms <= max_fallback_ms) endt=appc[s]
        }
      }
      if (endt!="") {
        hs_complete++
        push_hs((endt-ch[s])*1000)
        b=int(endt)
        bucket[b]++
      }
    }
    ...
    if (nlat>0) {
      asort(lat)
      printf "LAT_P50=%.3f\nLAT_P95=%.3f\nLAT_P99=%.3f\n", compute(lat,nlat,50), compute(lat,nlat,95), compute(lat,nlat,99)
    } else {
      printf "LAT_P50=\nLAT_P95=\nLAT_P99=\n"
    }
    printf "LAT_N=%d\n", nlat+0
    if (nlat>0) printf "LAT_AVG=%.3f\n", lat_sum/nlat; else printf "LAT_AVG=\n"

    if (nhs>0) {
      asort(hs)
      printf "HS_P50=%.3f\nHS_P95=%.3f\nHS_P99=%.3f\n", compute(hs,nhs,50), compute(hs,nhs,95), compute(hs,nhs,99)
    } else {
      printf "HS_P50=\nHS_P95=\nHS_P99=\n"
    }
    printf "HS_N=%d\n", nhs+0
    if (nhs>0) printf "HS_AVG=%.3f\n", hs_sum/nhs; else printf "HS_AVG=\n"
    printf "CH_TOTAL=%d\n", ch_total+0
    printf "HS_COMPLETE=%d\n", hs_complete+0

    # throughput por ventana temporal (1s)
    minb=""
    maxb=""
    for (b in bucket) {
      if (minb=="" || b<minb) minb=b
      if (maxb=="" || b>maxb) maxb=b
    }
    if (minb!="") {
      for (i=minb; i<=maxb; i++) {
        v=(i in bucket)?bucket[i]:0
        push_tput(v)
      }
    }
    if (ntput>0) {
      asort(tput)
      printf "HS_TPUT_GLOBAL=%.6f\n", hs_complete/ntput
      printf "HS_TPUT_P50=%.3f\nHS_TPUT_P95=%.3f\nHS_TPUT_P99=%.3f\n", compute(tput,ntput,50), compute(tput,ntput,95), compute(tput,ntput,99)
      printf "HS_TPUT_MAX=%.3f\n", tput_max+0
      printf "HS_TPUT_NWIN=%d\n", ntput+0
    } else {
      printf "HS_TPUT_GLOBAL=\nHS_TPUT_P50=\nHS_TPUT_P95=\nHS_TPUT_P99=\nHS_TPUT_MAX=\nHS_TPUT_NWIN=0\n"
    }
  }
' \
  <( { tshark -r "${PCAP}" -Y "${BASE_FILTER} && tls.handshake.type==1" -T fields -e tcp.stream -e frame.time_epoch 2>/dev/null || true; } ) \
  <( { tshark -r "${PCAP}" -Y "${BASE_FILTER} && tls.handshake.type==2" -T fields -e tcp.stream -e frame.time_epoch 2>/dev/null || true; } ) \
  <( { tshark -r "${PCAP}" -Y "${BASE_FILTER} && tls.handshake.type==20" -T fields -e tcp.stream -e frame.time_epoch 2>/dev/null || true; } ) \
  <( { tshark -r "${PCAP}" -Y "${BASE_FILTER} && (tls.app_data || tls.record.content_type==23) && ip.src==${PRIMARY_CLIENT_IP}" -T fields -e tcp.stream -e frame.time_epoch 2>/dev/null || true; } )
)
LAT_TLS_CH_TO_SH_P50_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^LAT_P50=/{print $2}')
LAT_TLS_CH_TO_SH_P95_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^LAT_P95=/{print $2}')
LAT_TLS_CH_TO_SH_P99_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^LAT_P99=/{print $2}')
HANDSHAKE_DURATION_P50_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_P50=/{print $2}')
HANDSHAKE_DURATION_P95_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_P95=/{print $2}')
HANDSHAKE_DURATION_P99_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_P99=/{print $2}')

LAT_TLS_CH_TO_SH_N=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^LAT_N=/{print $2}')
HANDSHAKE_DURATION_N=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_N=/{print $2}')
LAT_TLS_CH_TO_SH_AVG_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^LAT_AVG=/{print $2}')
HANDSHAKE_DURATION_AVG_MS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_AVG=/{print $2}')
HS_STREAMS_CH_TOTAL=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^CH_TOTAL=/{print $2}')
HS_STREAMS_COMPLETE=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_COMPLETE=/{print $2}')
HANDSHAKE_THROUGHPUT_GLOBAL_HSPS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_GLOBAL=/{print $2}')
HANDSHAKE_THROUGHPUT_P50_HSPS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_P50=/{print $2}')
HANDSHAKE_THROUGHPUT_P95_HSPS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_P95=/{print $2}')
HANDSHAKE_THROUGHPUT_P99_HSPS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_P99=/{print $2}')
HANDSHAKE_THROUGHPUT_PEAK_HSPS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_MAX=/{print $2}')
HANDSHAKE_THROUGHPUT_WINDOWS=$(printf "%s\n" "$PERCENTILES_OUTPUT" | awk -F= '/^HS_TPUT_NWIN=/{print $2}')
if [ -n "${HS_STREAMS_CH_TOTAL}" ] && [ "${HS_STREAMS_CH_TOTAL}" != "0" ]; then
  HANDSHAKE_SUCCESS_RATE_PCT=$(awk -v c="${HS_STREAMS_COMPLETE}" -v t="${HS_STREAMS_CH_TOTAL}" 'BEGIN{if ((t+0)>0) printf "%.3f", (c/t)*100; else print ""}')
else
  HANDSHAKE_SUCCESS_RATE_PCT=""
fi
if [ -n "${CLIENT_HELLO_TCP_BYTES}" ] && [ "${CLIENT_HELLO_TCP_BYTES}" != "0" ]; then
  HELLO_PAYLOAD_EXPANSION_RATIO=$(awk -v sh="${SERVER_HELLO_TCP_BYTES}" -v ch="${CLIENT_HELLO_TCP_BYTES}" 'BEGIN{if ((ch+0)>0) printf "%.6f", sh/ch; else print ""}')
else
  HELLO_PAYLOAD_EXPANSION_RATIO=""
fi
if [ -n "${CLIENT_HELLO_COUNT}" ] && [ "${CLIENT_HELLO_COUNT}" != "0" ]; then
  TLS_HS_BYTES_PER_CONNECTION=$(awk -v chb="${CLIENT_HELLO_TCP_BYTES}" -v shb="${SERVER_HELLO_TCP_BYTES}" -v n="${CLIENT_HELLO_COUNT}" 'BEGIN{if ((n+0)>0) printf "%.3f", (chb+shb)/n; else print ""}')
else
  TLS_HS_BYTES_PER_CONNECTION=""
fi
TLS_HS_BYTES_PER_CONNECTION_P50=$( { tshark -r "${PCAP}" -Y "${BASE_FILTER} && (tls.handshake.type==1 || tls.handshake.type==2)" -T fields -e tcp.stream -e tls.handshake.type -e tcp.len 2>/dev/null || true; } \
  | awk '
      NF>=3 {
        s=$1; t=$2; l=$3+0
        if (t=="1") ch[s]+=l
      ...
      END {
        ...
        lo=int(rank); hi=lo+1; if (hi>n) hi=n
        frac=rank-lo
        p=vals[lo] + frac*(vals[hi]-vals[lo])
        printf "%.3f", p
      }
    ')
if [ -n "${HANDSHAKE_DURATION_AVG_MS}" ] && [ -n "${PQC_CLIENT_CPU_AVG}" ] && [ "${PQC_CLIENT_CPU_AVG}" != "NA" ] && [ "${PQC_CLIENT_CPU_AVG}" != "0" ]; then
  NET_CPU_CLIENT_RATIO=$(awk -v l="${HANDSHAKE_DURATION_AVG_MS}" -v c="${PQC_CLIENT_CPU_AVG}" 'BEGIN{if ((c+0)>0) printf "%.6f", l/c; else print ""}')
else
  NET_CPU_CLIENT_RATIO=""
fi
if [ -n "${HANDSHAKE_DURATION_AVG_MS}" ] && [ -n "${PQC_SERVER_CPU_AVG}" ] && [ "${PQC_SERVER_CPU_AVG}" != "NA" ] && [ "${PQC_SERVER_CPU_AVG}" != "0" ]; then
  NET_CPU_SERVER_RATIO=$(awk -v l="${HANDSHAKE_DURATION_AVG_MS}" -v c="${PQC_SERVER_CPU_AVG}" 'BEGIN{if ((c+0)>0) printf "%.6f", l/c; else print ""}')
else
  NET_CPU_SERVER_RATIO=""
fi
HANDSHAKE_MIN_SAMPLES="${HANDSHAKE_MIN_SAMPLES:-30}"
HANDSHAKE_DURATION_QUALITY="ok"
if [ -z "${HANDSHAKE_DURATION_N}" ]; then
  HANDSHAKE_DURATION_QUALITY="no_samples"
elif [ "${HANDSHAKE_DURATION_N}" -lt "${HANDSHAKE_MIN_SAMPLES}" ]; then
  HANDSHAKE_DURATION_QUALITY="insufficient_samples"
fi
{
  echo "=== INFO GENERAL ==="
  echo "RUN_ID: ${RUN_ID}"
  ...
  echo "TCP ACK RTT p50 (ms): ${TCP_ACK_RTT_P50_MS}"
  echo "TCP ACK RTT p95 (ms): ${TCP_ACK_RTT_P95_MS}"
  echo "TCP ACK RTT p99 (ms): ${TCP_ACK_RTT_P99_MS}"
  echo "Lat TCP SYN -> SYN-ACK (ms): ${LAT_TCP_SYN_TO_SYNACK_MS}"
  echo "Lat TCP SYN-ACK -> ACK (ms): ${LAT_TCP_SYNACK_TO_ACK_MS}"
  echo "Lat TLS ClientHello -> ServerHello (ms, primary stream): ${LAT_TLS_CH_TO_SH_MS}"
  echo "Lat TLS ClientHello -> ServerHello avg (ms, all streams): ${LAT_TLS_CH_TO_SH_AVG_MS}"
  echo "Lat TLS ClientHello -> ServerHello p50 (ms): ${LAT_TLS_CH_TO_SH_P50_MS}"
  echo "Lat TLS ClientHello -> ServerHello p95 (ms): ${LAT_TLS_CH_TO_SH_P95_MS}"
  echo "Lat TLS ClientHello -> ServerHello p99 (ms): ${LAT_TLS_CH_TO_SH_P99_MS}"
  echo "Lat TLS ClientHello -> ServerHello samples (N): ${LAT_TLS_CH_TO_SH_N}"
  ...
  echo "Handshake duration ms (approx, primary stream): ${HANDSHAKE_DURATION_MS}"
  echo "Handshake duration avg ms (approx, sampled streams): ${HANDSHAKE_DURATION_AVG_MS}"
  echo "Handshake duration p50 ms (approx): ${HANDSHAKE_DURATION_P50_MS}"
  echo "Handshake duration p95 ms (approx): ${HANDSHAKE_DURATION_P95_MS}"
  echo "Handshake duration p99 ms (approx): ${HANDSHAKE_DURATION_P99_MS}"
  echo "Handshake duration samples (N): ${HANDSHAKE_DURATION_N}"
  echo "Handshake duration sample threshold: ${HANDSHAKE_MIN_SAMPLES}"
  echo "Handshake duration percentile quality: ${HANDSHAKE_DURATION_QUALITY}"
  echo "Handshake fallback max gap ms for percentiles: ${HS_APPDATA_FALLBACK_MAX_MS}"
  echo "Handshake streams with CH: ${HS_STREAMS_CH_TOTAL}"
  echo "Handshake streams complete (CH+SH+fin heurístico): ${HS_STREAMS_COMPLETE}"
  echo "Handshake success rate (%): ${HANDSHAKE_SUCCESS_RATE_PCT}"
  echo "Handshake throughput global (hs/s): ${HANDSHAKE_THROUGHPUT_GLOBAL_HSPS}"
  echo "Handshake throughput p50 (hs/s, ventana ${THROUGHPUT_WINDOW_S}s): ${HANDSHAKE_THROUGHPUT_P50_HSPS}"
  echo "Handshake throughput p95 (hs/s, ventana ${THROUGHPUT_WINDOW_S}s): ${HANDSHAKE_THROUGHPUT_P95_HSPS}"
  echo "Handshake throughput p99 (hs/s, ventana ${THROUGHPUT_WINDOW_S}s): ${HANDSHAKE_THROUGHPUT_P99_HSPS}"
  echo "Handshake throughput peak (hs/s): ${HANDSHAKE_THROUGHPUT_PEAK_HSPS}"
  echo "Handshake throughput ventanas usadas: ${HANDSHAKE_THROUGHPUT_WINDOWS}"
  echo "Crypto cost ratio (ServerHello_tcp_bytes / ClientHello_tcp_bytes): ${HELLO_PAYLOAD_EXPANSION_RATIO}"
  echo "TLS hello bytes por conexión (tcp payload): ${TLS_HS_BYTES_PER_CONNECTION}"
  echo "TLS hello bytes por conexión p50 (tcp payload): ${TLS_HS_BYTES_PER_CONNECTION_P50}"
  echo "Red-vs-CPU ratio cliente (handshake_ms_avg / cpu_avg): ${NET_CPU_CLIENT_RATIO}"
  echo "Red-vs-CPU ratio servidor (handshake_ms_avg / cpu_avg): ${NET_CPU_SERVER_RATIO}"
...
{
  echo "run_id,scenario,group_name,packets_total,bytes_total,syn_count,synack_count,ack_only_count,fin_count,rst_count,retransmissions,fast_retransmissions,spurious_retransmissions,duplicate_ack,out_of_order,lost_segments,tls_handshake_total,tls_alerts,clienthello_count,serverhello_count,encryptedextensions_count,certificate_count,certificateverify_count,finished_count,clienthello_packets,serverhello_packets,clienthello_frame_bytes,serverhello_frame_bytes,clienthello_tcp_bytes,serverhello_tcp_bytes,max_tcp_payload_bytes,max_ip_packet_bytes,max_eth_frame_bytes,tcp_mss_client,tcp_mss_server,tcp_mss_negotiated,mtu_assumed_bytes,mtu_exceeded_ip_packets,mtu_exceeded_c2s,mtu_exceeded_s2c,mss_exceeded_segments,mss_exceeded_ip_packets,mss_exceeded_c2s,mss_exceeded_s2c,mss_client_unique,mss_server_unique,ip_fragmented_count,tcp_near_mss_low,tcp_near_mss_count,ch_streams,ch_packets_per_stream_avg,sh_streams,sh_packets_per_stream_avg,tcp_ack_rtt_avg_ms,tcp_ack_rtt_max_ms,tcp_ack_rtt_p50_ms,tcp_ack_rtt_p95_ms,tcp_ack_rtt_p99_ms,lat_tcp_syn_to_synack_ms,lat_tcp_synack_to_ack_ms,lat_tls_ch_to_sh_ms,lat_tls_ch_to_sh_p50_ms,lat_tls_ch_to_sh_p95_ms,lat_tls_ch_to_sh_p99_ms,lat_tls_ch_to_sh_n,lat_tls_sh_to_fin_ms,lat_tls_fin_to_appdata_ms,handshake_duration_ms_pcap,handshake_duration_p50_ms_pcap,handshake_duration_p95_ms_pcap,handshake_duration_p99_ms_pcap,handshake_duration_n_pcap,handshake_duration_quality,handshake_streams_ch_total,handshake_streams_complete,handshake_success_rate_pct,handshake_throughput_global_hsps,handshake_throughput_p50_hsps,handshake_throughput_p95_hsps,handshake_throughput_p99_hsps,handshake_throughput_peak_hsps,hello_payload_expansion_ratio,tls_hs_bytes_per_connection,tls_hs_bytes_per_connection_p50,net_cpu_client_ratio,net_cpu_server_ratio,resource_summary_present,resource_csv_present,pqc_client_cpu_avg,pqc_client_cpu_max,pqc_client_mem_avg,pqc_client_mem_max,pqc_server_cpu_avg,pqc_server_cpu_max,pqc_server_mem_avg,pqc_server_mem_max,pqc_monitor_cpu_avg,pqc_monitor_cpu_max,pqc_monitor_mem_avg,pqc_monitor_mem_max"
  echo "${RUN_ID},${SCENARIO},${GROUP_NAME},${PACKETS_TOTAL},${BYTES_TOTAL},${SYN_COUNT},${SYNACK_COUNT},${ACK_ONLY_COUNT},${FIN_COUNT},${RST_COUNT},${RETRANS_COUNT},${FAST_RETRANS_COUNT},${SPURIOUS_RETRANS_COUNT},${DUP_ACK_COUNT},${OUT_OF_ORDER_COUNT},${LOST_SEG_COUNT},${TLS_HS_TOTAL},${TLS_ALERTS},${CLIENT_HELLO_COUNT},${SERVER_HELLO_COUNT},${ENCRYPTED_EXT_COUNT},${CERTIFICATE_COUNT},${CERT_VERIFY_COUNT},${FINISHED_COUNT},${CLIENT_HELLO_PACKETS},${SERVER_HELLO_PACKETS},${CLIENT_HELLO_FRAME_BYTES},${SERVER_HELLO_FRAME_BYTES},${CLIENT_HELLO_TCP_BYTES},${SERVER_HELLO_TCP_BYTES},${MAX_TCP_PAYLOAD_BYTES},${MAX_IP_PACKET_BYTES},${MAX_ETH_FRAME_BYTES},${TCP_MSS_CLIENT},${TCP_MSS_SERVER},${TCP_MSS_NEGOTIATED},${MTU_ASSUMED_BYTES},${MTU_EXCEEDED_IP_PACKETS},${MTU_EXCEEDED_C2S},${MTU_EXCEEDED_S2C},${MSS_EXCEEDED_SEGMENTS},${MSS_EXCEEDED_IP_PACKETS},${MSS_EXCEEDED_C2S},${MSS_EXCEEDED_S2C},${MSS_CLIENT_UNIQUE},${MSS_SERVER_UNIQUE},${IP_FRAGMENTED_COUNT},${TCP_NEAR_MSS_LOW},${TCP_NEAR_MSS_COUNT},${CH_STREAMS},${CH_PACKETS_PER_STREAM_AVG},${SH_STREAMS},${SH_PACKETS_PER_STREAM_AVG},${TCP_ACK_RTT_AVG_MS},${TCP_ACK_RTT_MAX_MS},${TCP_ACK_RTT_P50_MS},${TCP_ACK_RTT_P95_MS},${TCP_ACK_RTT_P99_MS},${LAT_TCP_SYN_TO_SYNACK_MS},${LAT_TCP_SYNACK_TO_ACK_MS},${LAT_TLS_CH_TO_SH_MS},${LAT_TLS_CH_TO_SH_P50_MS},${LAT_TLS_CH_TO_SH_P95_MS},${LAT_TLS_CH_TO_SH_P99_MS},${LAT_TLS_CH_TO_SH_N},${LAT_TLS_SH_TO_FIN_MS},${LAT_TLS_FIN_TO_APPDATA_MS},${HANDSHAKE_DURATION_MS},${HANDSHAKE_DURATION_P50_MS},${HANDSHAKE_DURATION_P95_MS},${HANDSHAKE_DURATION_P99_MS},${HANDSHAKE_DURATION_N},${HANDSHAKE_DURATION_QUALITY},${HS_STREAMS_CH_TOTAL},${HS_STREAMS_COMPLETE},${HANDSHAKE_SUCCESS_RATE_PCT},${HANDSHAKE_THROUGHPUT_GLOBAL_HSPS},${HANDSHAKE_THROUGHPUT_P50_HSPS},${HANDSHAKE_THROUGHPUT_P95_HSPS},${HANDSHAKE_THROUGHPUT_P99_HSPS},${HANDSHAKE_THROUGHPUT_PEAK_HSPS},${HELLO_PAYLOAD_EXPANSION_RATIO},${TLS_HS_BYTES_PER_CONNECTION},${TLS_HS_BYTES_PER_CONNECTION_P50},${NET_CPU_CLIENT_RATIO},${NET_CPU_SERVER_RATIO},${RESOURCE_SUMMARY_PRESENT},${RESOURCE_CSV_PRESENT},${PQC_CLIENT_CPU_AVG},${PQC_CLIENT_CPU_MAX},${PQC_CLIENT_MEM_AVG},${PQC_CLIENT_MEM_MAX},${PQC_SERVER_CPU_AVG},${PQC_SERVER_CPU_MAX},${PQC_SERVER_MEM_AVG},${PQC_SERVER_MEM_MAX},${PQC_MONITOR_CPU_AVG},${PQC_MONITOR_CPU_MAX},${PQC_MONITOR_MEM_AVG},${PQC_MONITOR_MEM_MAX}"
} > "${OUT_CSV}"
