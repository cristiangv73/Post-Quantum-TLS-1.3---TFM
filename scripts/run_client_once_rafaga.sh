...
echo "[*] Ráfaga clásica completada: ${TOTAL_CONN} conexiones en ${clients_count} clientes (${ARRIVAL_PROFILE})"
echo "=== host results ==="
ls -l results | grep "${RUN_ID}" || true
echo "=== resumen por cliente ==="
sed -n "1,200p" "results/${RUN_ID}_client.txt" || true
