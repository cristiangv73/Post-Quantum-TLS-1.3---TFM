...
run_one() {
  ...
  run_id="hybrid_${p_lc}_${scenario}_c${total_conn}_${idx}"
  RUN_IDS+=("$run_id")
  ...
  if [[ "$scenario" == "stress" ]]; then
    CLIENTS="$CLIENTS" ARRIVAL_PROFILE=stress \
      ...
      bash -lc "./scripts/run_client_once_rafaga_hibrido.sh ${run_id} ${total_conn}; sleep 3"
  else
    local pr jm
...
LOG_FILE="logs/campaign_hybrid_$(date +%Y%m%d_%H%M%S).log"
echo "[$(ts)] Iniciando campaña híbrida. Log: ${LOG_FILE}"
...
echo "[$(ts)] Campaña híbrida finalizada OK."
